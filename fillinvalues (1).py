...
model1 = ols("homes_for_sale ~ average_price + months_supply + unemployment_rate + median_income", data= house_copy).fit()
print(model1.summary())

# second model that takes into account if the value was filled in or not
model2 = ols("homes_for_sale ~ average_price + months_supply + unemployment_rate + median_income + C(income_was_missing)", data= house_copy).fit()
print(model2.summary())

# second model has an improved r-squared score